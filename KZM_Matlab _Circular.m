...
%Solving the equation
        ...
        for i=2:Ns
            ...
            for k=1:n
                if k==1
                    Ps(1)=Pt(1,i-1)-(h*Pt(1,i-1)+c*(Xt(2,i-1) - Xt(n,i-1))+f*(Xt(1,i-1)*(-2*(t./tq)+1)+Xt(1,i-1)^3) )*dt + s*Wn(1,i-1)*sqrt(dt);
                    Xs(1)=Xt(1,i-1)+Pt(1,i-1)*dt;
                else
                    if k==n
                        Ps(n)=Pt(n,i-1)-(h*Pt(n,i-1)+c*(Xt(1,i-1) - Xt(n-1,i-1))+f*(Xt(n,i-1)*(-2*(t./tq)+1)+Xt(n,i-1)^3) )*dt + s*Wn(n,i-1)*sqrt(dt);
